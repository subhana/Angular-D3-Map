		...
		drawMapModule.directive('sfDrawMap', function () {
			return {
                ...
                link: function(scope, elm, attrs) {
					...
					function ready(error, neighborhoods, freeways) {
					...
					//	console.log(streets);
                        ...
                        svg.selectAll("path").data(neighborhoods.features).enter().append("path").attr("d", path).attr('data-id', function(d) {
                            return d.id;
                        }).attr('data-name', function(d) {
                            return d.properties.name;
                        }).style("fill", "#FB5B1F").style("stroke", "#ffffff");

						svg.selectAll("path").data(freeways.features).enter().append("path").attr("d", path).attr('data-id', function(d) {
                            return d.id;
                        }).attr('data-name', function(d) {
                            return d.properties.name;
                        }).style("fill", "#FB5B1F").style("stroke", "black");
